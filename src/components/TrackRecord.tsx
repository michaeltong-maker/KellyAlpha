...
import { buildTrackRecord, groupStats, totalAvg, firstCallDate, benchmarkReturn, type Signal } from '../lib/trackRecord';
...
export function TrackRecord({ agentId }: { agentId: string }) {
  ...
  const groups = useMemo(() => groupStats(calls), [calls]);
  ...
  return (
    <section className="px-4 py-3 border-b border-ink-200">
      ...
      <div className="rounded-lg border border-ink-200 p-4">
        ...
        {/* Per-call table, grouped under signal dividers */}
            ...
            <tbody>
              {groups.map((g) => (
                <Fragment key={g.signal}>
                  {/* Signal divider row with win rate + avg */}
                  <tr>
                    <td colSpan={6} className="pt-3 pb-2">
